...
3. **PULL_REQUEST_EXPLANATION & SCOPING**: Pull Requests MUST be created via `gh pr create` with Metadata Header Banner (`Resolved by Agent`, `agent_model`, `session_id`, `log_file`), PR label auto-creation (`gh label create`), attached `--label "<agent_name>"`, root cause explanation, fix details, test verification, and issue linkage (`Fixes #<issue_number>`).
   - **Avoid Low-Scoped PRs**: PRs must **not** be artificially low-scoped. A PR can and should contain a complete, coherent set of related fixes, enhancements, tests, and accompanying documentation together if they are related to the same functional milestone or issue set. Do not fragment related changes into multiple micro-PRs.
