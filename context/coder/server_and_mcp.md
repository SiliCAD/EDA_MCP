...
## 1. Instance & Logging Setup ([`server.py`](../../src/server.py))
...
To prevent working directory (`cd`) or environment contamination across tools, [`src/server.py`](../../src/server.py#L58-L66) instantiates separate `RemoteSession` instances:
...
## 4. Background Daemon Architecture (`daemon.py` & `eda_client.py`)
